...
from transformers.models.blenderbot_small.modeling_blenderbot_small import BlenderbotSmallModel
...
from .attentions import KeyValueAttention, _expand_mask, _prepare_decoder_attention_mask
from .causal_encoders import BlenderbotSmallCausalEncoder
from .multi_factor_decoders import BlenderbotSmallMultiFactorDecoder
from .PARAMS import ALPHA, SAMPLE, TEMPERATURE, STRATEGY_LABELS
...
class BlenderbotSmallCausalModel(BlenderbotSmallModel):
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        self.encoder = BlenderbotSmallCausalEncoder(config, self.shared)
        self.decoder = BlenderbotSmallMultiFactorDecoder(config, self.shared)
...
class Model(BaseModel, BlenderbotSmallForConditionalGeneration):
    ...
    def register_strategy(self):
        batch_encoding = self.toker(list(STRATEGY_LABELS), padding=True, return_tensors='pt')
    ...
    def get_encoder_outputs(
        ...
        situation_encoder_outputs: BaseModelOutput = self.model.encoder(
            ...
            causal_attention_mask=situation_attention_mask,
            head_mask=head_mask,
        ...
        x4, x5, x6, x1, x2, x3, o1, o2, o3 = situation_features # torch.Size([bs, dim])
        ...
        strategy_hidden_states = strategy_hidden_states[None, ...].expand(batch_size, -1, -1, -1) # torch.Size([bs, 8, len, dim])
        strategy_hidden_states = strategy_score[:, :, None, None] * strategy_hidden_states
        strategy_hidden_states = strategy_hidden_states.flatten(start_dim=1, end_dim=2) # torch.Size([bs, 8*len, dim])
