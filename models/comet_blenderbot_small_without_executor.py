...
from transformers.models.blenderbot_small.modeling_blenderbot_small import BlenderbotSmallModel, BlenderbotSmallDecoder
...
class Model(BaseModel, BlenderbotSmallForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.training or validation:
            ce_loss = F.cross_entropy(lm_logits[:,0,-8:], strategy_id)
            pred_strategy_id = torch.argmax(lm_logits[:,0,-8:], dim=-1)
    ...
    def generate(
        ...
        **kwargs
        ...
        decoder_input_ids = torch.cat([decoder_input_ids, pred_strategy[..., None] + len(self.toker) - 8], dim=-1)
