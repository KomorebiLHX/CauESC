...
from utils.building_utils import to_device
...
def eval_model_loss(model, eval_dataloader, epoch_id, infer, args):
    if 'comet' in args.config_name or 'causal' in args.config_name:
        # calc acc
        ...
        with torch.no_grad():
            for step, batch in enumerate(eval_dataloader):
                ...
                loss_sample, n_sample, strat_pred, strat_true = model(
                    validation=True,
                    **batch
                )
                ...
                pointwise_pred.extend(strat_pred.tolist())
                pointwise_true.extend(strat_true.tolist())
                if infer:
        ...
        mean_acc = accuracy_score(pointwise_pred, pointwise_true)
        logger.info(f"\nEpoch {epoch_id+1}: Val loss {mean_loss:.4f} Val ppl {mean_ppl:.4f} Val acc {mean_acc:.4f}")
        return mean_loss, mean_ppl, mean_acc, tot_sample, pointwise_loss, pointwise_sample
    else:
        # use the same signature with eval_model_generation
        logger.info('compute eval model loss, using eval mode, '
                    'please change it back to train after calling this function')
        ...
        mean_loss = tot_loss / tot_sample
        mean_ppl = np.exp(mean_loss)
        logger.info(f"\nEpoch {epoch_id+1}: Val loss {mean_loss} Val ppl {mean_ppl}")
