...
def build_model(only_toker=False, checkpoint=None, local_rank=-1, **kwargs):
    ...
    if config.get('custom_config_path', None) is not None:
        custom_config = AutoConfig.from_pretrained(config['custom_config_path'])
        model = Model.from_pretrained(config['pretrained_model_path'], config=custom_config)
    else:
...
def to_device(batch, device):
    ...
    if 'situation_features' in batch:
        batch['situation_features'] = [i.to(device) for i in batch['situation_features']]
    if 'utterance_features' in batch:
        batch['utterance_features'] = [i.to(device) for i in batch['utterance_features']]
    if 'comet_features' in batch:
        batch['comet_features'] = [i.to(device) for i in batch['comet_features']]
    return batch