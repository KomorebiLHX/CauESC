...
from utils.building_utils import boolean_string, build_model, deploy_model, to_device
...
logger.info('Generation Argument Information')
...
for infer_idx, infer_input_file in enumerate(args.infer_input_file):
    ...
    if not args.only_encode and not args.only_generate:
        ...
        infer_loss, *others = eval_model_loss(
            model=model,
        ...
        infer_samples, pointwise_loss, pointwise_sample = others[-3:]
    ...
    res = []
    other_res = {}
    decode = lambda x: _norm(toker.decode(x, skip_special_tokens=True))
        ...
        if 'other_res' in batch:
            ...
            if add_acc:
                ...
                for k, v in batch_other_res['acc_map'].items():
                    ...
                    for i in range(1, 6):
                        if f'{v}_top{i}' in encoded_info:
                            encoded_info[f'{v}_top{i}'] = encoded_info[f'{v}_top{i}'].tolist()
                    ...
                    for i in range(1, 6):
                        if f'{v}_top{i}' in encoded_info:
                            if f'{v}_top{i}' not in other_res['acc']:
                                other_res['acc'][f'{v}_top{i}'] = []
                            other_res['acc'][f'{v}_top{i}'].extend(encoded_info[f'{v}_top{i}'])
        ...
        for idx in range(len(sample_ids)):
            ...
            if not args.only_encode:
                ...
                if not args.only_generate and args.num_return_sequences == 1:
                    ref, gen = [r], toker.decode(g, skip_special_tokens=True) if not isinstance(g[0], list) else toker.decode(g[0], skip_special_tokens=True)
            ...
            if batch_other_res is not None:
                if add_acc:
                    for k, v in batch_other_res['acc_map'].items():
                        if k not in batch_other_res or v not in encoded_info: # TODO
                            continue # TODO
                        other_res_to_append[k] = batch_other_res[k][idx]
                        other_res_to_append[v] = encoded_info[v][idx]
                        for i in range(1, 6):
                            if f'{v}_top{i}' in encoded_info:
                                other_res_to_append[f'{v}_top{i}'] = encoded_info[f'{v}_top{i}'][idx]
                        if f'{v}_dist' in encoded_info:
    ...
    if not args.only_encode:
        save_dir = f'{checkpoint_dir_path}/res_{checkpoint_name}_{infer_input_file_name}_k.{args.top_k}' \
                   ...
                   f'_rp.{args.repetition_penalty}_ng.{args.no_repeat_ngram_size}_ts.{timestamp}'
    else:
    ...
    if not args.only_generate:
        if 'acc' in other_res:
            for k, v in other_res['acc_map'].items():
                ...
                print(f'{k}: classification_report\n', classification_report(kk, vv, zero_division=0))
                ...
                for i in range(1, 6):
                    if f'{v}_top{i}' in other_res['acc']:
                        vv_top_i = np.array(other_res['acc'][f'{v}_top{i}'], dtype=int)
                        metric_res[f'acc_{k}_top{i}'] = np.mean(np.sum((kk.reshape(-1, 1) - vv_top_i) == 0, axis=-1) != 0)
                        metric_res_list[f'acc_{k}_top{i}'] = (np.sum((kk.reshape(-1, 1) - vv_top_i) == 0, axis=-1) != 0).astype(int).tolist()
    ...
    logger.info(f"saved metrics in {save_dir}")
